...
def fused_add_tanh_sigmoid_multiply(input_a, input_b, n_channels):
    Bb, Cb, Hb, Wb = input_b.shape
    input_a = input_a[:Bb,:Cb,:Hb,:Wb]
...
class SqueezeLayer(nn.Module):
    def __init__(self, scale_h):
        super().__init__()
        self.sh = scale_h

    def forward(self, x, log_det=None):
        x = self.squeeze(x, self.sh)
        x = x.unsqueeze(1)
        if log_det is None:
            return x
        return x, log_det
    ...
    def reverse(self, z):
        z = z.squeeze(1)
        z = self.unsqueeze(z, self.sh)

        return z

    def squeeze(self, x, sh):
        B, C, T = x.size()
        squeezed_x = x.contiguous().view(B, C, T // sh, sh).permute(0, 1, 3, 2)
        squeezed_x = squeezed_x.contiguous().view(B, C * sh, T // sh)

        return squeezed_x

    def unsqueeze(self, z, sh):
        B, C, T = z.size()
        unsqueezed_z = z.view(B, C // sh, sh, T).permute(0, 1, 3, 2)
        unsqueezed_z = unsqueezed_z.contiguous().view(B, C // sh, T * sh)
...
class WaveNetDiscreteVer2(nn.Module):
    def __init__(self, n_in_channels, n_mel_channels, n_out_channels, d_i_horizon, d_i_vertical, n_layers, n_channels, kernel_size=3, direction=0):
        super().__init__()
        ...
        end = torch.nn.Conv2d(n_channels, 2 * n_out_channels, 1)
        ...
        for i in range(n_layers):
            dilation_h = d_i_horizon ** i
            dilation_v = d_i_vertical ** i
            if direction % 2 == 0:
                padding_horizon = int((kernel_size*dilation_h - dilation_h)/2)
                padding_vertical =  int((kernel_size*dilation_v - dilation_v)/2) * 2
            else:
                padding_horizon = int((kernel_size*dilation_h - dilation_h)/2) * 2
                padding_vertical =  int((kernel_size*dilation_v - dilation_v)/2) 
            in_layer = torch.nn.Conv2d(n_channels, 2*n_channels, kernel_size,
                                       dilation=[dilation_v, dilation_h], padding=[padding_vertical, padding_horizon])
...
class WaveNetDiscrete(nn.Module):
    def __init__(self, n_in_channels, n_mel_channels, n_out_channels, d_i, n_layers, n_channels, kernel_size=3, direction=0):
        super().__init__()
        assert(kernel_size % 2 == 1)
        assert(n_channels % 2 == 0)
        self.n_layers = n_layers
        ...
        for i in range(n_layers):
            dilation = d_i ** i
            if direction % 2 == 0:
                padding_horizon = int((kernel_size*dilation - dilation)/2)
                padding_vertical =  int((kernel_size*dilation - dilation)/2) * 2
            else:
                padding_horizon = int((kernel_size*dilation - dilation)/2) * 2
                padding_vertical =  int((kernel_size*dilation - dilation)/2) 
            in_layer = torch.nn.Conv2d(n_channels, 2*n_channels, kernel_size,
                                       dilation=dilation, padding=[padding_vertical, padding_horizon])
            in_layer = torch.nn.utils.weight_norm(in_layer, name='weight')
            self.in_layers.append(in_layer)
