...
def tanh_sigmoid_activation(n_channels, input_a, input_b, input_c, input_d=None):
...
class WaveNet(nn.Module):
    def __init__(self, in_channels, out_channels, cin_channels, di_cycle, pos_group, n_channels, n_layers, kernel_size=3):
        super().__init__()
        ...
        std_layer = torch.nn.Linear(1, 2*n_channels*n_layers)
        self.std_layer = torch.nn.utils.weight_norm(std_layer, name='weight')
    ...
    def forward(self, audio, spect, std, pos=None):
        audio = self.start(audio)
        output = torch.zeros_like(audio)
        n_channels_tensor = torch.IntTensor([self.n_channels])

        spect = self.cond_layer(spect)
        std = self.std_layer(std)
        ...
        for i in range(self.n_layers):
            ...
            std_in = std[:, spect_offset:spect_offset+2*self.n_channels, :]
            pos_in = pos[:, pos_offset:pos_offset+2*self.n_channels, :] if pos is not None else None
            acts = tanh_sigmoid_activation(n_channels_tensor, self.in_layers[i](audio), spect_in, std_in, pos_in)
...
class Invertible1x1Conv(torch.nn.Module):
    ...
    def forward(self, x, c, std, log_det_sum):
        # shape
        B_, _, T_ = x.size()
...
# class FullResolutionFlow(nn.Module):
#     def __init__(self, in_channels, cin_channels, dilation, pos_group, n_channels, n_layers):
#         super().__init__()
#         self.pos_group = pos_group
#         self.in_channels = in_channels
#         self.WN = WaveNet(cin_channels, 2*in_channels, 0, dilation, 0, n_channels, n_layers)

#     def forward(self, x, c, log_det_sum):
#         pos = None
#         cond = None
#         log_s, b = self.WN(c, cond, pos)
#         z = torch.exp(log_s) * x + b
...
class PosConditionedFlow(nn.Module):
    def __init__(self, in_channels, cin_channels, dilation, pos_group, n_channels, n_layers):
        ...
        self.pos_group = pos_group
        self.in_channels = in_channels
        self.WN = WaveNet(in_channels//2, in_channels, cin_channels, dilation, pos_group, n_channels, n_layers)
...
class EqualResolutionBlock(nn.Module):
    ...
    def forward(self, x, c, std, log_det_sum):
        for chain in self.chains:
            x, c, log_det_sum = chain(x, c, std, log_det_sum)
        z = x
...
class SmartVocoder(nn.Module):
    ...
    def build_ER_block(self, n_flow_blocks, in_channels, cin_channels, di_cycle, pos_group, n_channels, n_layers, pretrained):
        ...
        for i in range(n_flow_blocks):
            ...
            # if i == 0:
            #     chains += [FullResolutionFlow(in_channels, cin_channels, di_cycle, pos_group, n_channels, n_layers)]
    ...
    def forward(self, x, mel, std):
        ...
        for i, block in enumerate(self.ER_blocks):
            out, log_det_sum = block(out, c_in, _std, log_det_sum)
            ...
            if i != len(self.ER_blocks) -1:
                ...
                _std = torch.repeat_interleave(std, dim=0, repeats=sc**(i+1))
        z = out
    ...
    def reverse(self, z, mel, std):
        ...
        _std = torch.repeat_interleave(std, dim=0, repeats=sc**(len(self.ER_blocks)-1))
        for i, block in enumerate(self.ER_blocks[::-1]):
            out, _ = block.module.reverse(out, c_in, _std)
            ...
            if i != len(self.ER_blocks)-1 :
                ...
                _std = torch.repeat_interleave(std, dim=0, repeats=sc**(len(self.ER_blocks)-2-i))
